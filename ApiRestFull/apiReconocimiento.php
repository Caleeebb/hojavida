...
switch ($accion) {
    case 'create':
        try {
            ...
            $inputs = $_POST;
            $sql = "
            INSERT INTO reconocimiento(periodo, empresa, descripcion, persona_id) VALUES(:periodo, :empresa, :descripcion, :persona_id)
            ";
    ...
    case 'update':
        try {
            ...
            $sql = "UPDATE reconocimiento set $fields where id = $postID";
            $statement = $dbConnection->prepare($sql);
            bindAllValues($statement, $inputs);
            $statement->execute();
        ...
        case 'read':
            ...
            $sql = $dbConnection->prepare("SELECT * from reconocimiento");
            $sql->execute();
    ...
    case 'delete':
        try {
            $dbConnection->beginTransaction();
            $inputs = $_POST;
            $statement = $dbConnection->prepare("Delete from reconociento where id = :id");
